...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['camp-manager', 'transportation-coordinator', 'group-manager', 'staff-recruiter'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<CampInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/camps` : null),
    () => getCamps({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Camps',
      path: isTenantUser ? `/camps/view/${data?.[0]?.id}` : '/camps',
      entity: 'camp',
